...
SERVICES_TO_ENABLE="apparmor iptables ip6tables iwd systemd-networkd systemd-resolved docker chronyd systemd-homed fstrim.timer mydenyusb da-lockout-clear-tpm"
...
# Core.
PACSTRAP_PACKAGES="base ${BASE_DEVEL_WITHOUT_SUDO} ${KERNEL} ${KERNEL}-headers ${CPU_VENDOR}-ucode man-db iwd bcachefs-tools btrfs-progs chrony sbsigntools opendoas apparmor dracut binutils elfutils tpm2-tools sbctl linux-firmware"

if [[ ! -f "/tmp/arch_install_sh_devmode" ]]; then
  # Development tools.
  PACSTRAP_PACKAGES+=" jq yq clang go rust python-pip python-requests python-beautifulsoup4 python-html5lib dbeaver helix code patchutils nano cargo "
  
  # General system utils.
  PACSTRAP_PACKAGES+=" wireguard-tools pacman-contrib zip net-tools gptfdisk openssh pamixer uutils-coreutils fastfetch git unzip unrar pipewire-jack power-profiles-daemon python-gobject sof-firmware wireplumber pipewire-pulse pavucontrol"
  
  # Browsers, multimedia applications and fonts.
  PACSTRAP_PACKAGES+=" chromium ttf-opensans ttf-fantasque-sans-mono  zathura zathura-pdf-poppler ncspot alacritty "
  
  # Cloud, infrastructure and virtualization.
  PACSTRAP_PACKAGES+=" kubectl skaffold helm kustomize docker-buildx istio vagrant qemu-base dnsmasq libvirt bridge-utils docker docker-compose terraform minikube azure-cli aws-cli helm k9s "
  
  # Security tools.
  PACSTRAP_PACKAGES+=" checksec bubblewrap-suid peda valgrind gef radare2 arti openbsd-netcat sqlmap rustscan nmap arch-repro-status"
  ...
  # PACSTRAP_PACKAGES+=" flameshot qt6-multimedia-ffmpeg grim xdg-desktop-portal xdg-desktop-portal-wlr kanshi waybar hyprland mako swayidle swaylock brightnessctl xdg-desktop-portal-wlr slurp vulkan-validation-layers vulkan-icd-loader vulkan-headers vulkan-tools"
fi
